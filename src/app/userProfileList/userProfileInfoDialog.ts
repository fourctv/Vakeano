...
export class UserProfileInfoDialog extends RecordEditWindow
    ...
    @ViewChild(ProfileGenesInfo, { static: false })
    profileGenesInfo: ProfileGenesInfo;
    @ViewChild(ViewerContentInfo, { static: false })
    viewerContentInfo: ViewerContentInfo;
    @ViewChild(ProfileRecommendationsInfo, { static: false })
    profileRecommendationsInfo: ProfileRecommendationsInfo;
        ...
        if (this.currentRecord.isRecordLoaded()) {
            this.dialog.setTitle(
                "Profile Details: " +
                    ...
                    " for " +
                    this.currentRecord.UserName
            );
        } else {
            this.dialog.setTitle(
                "New Taste Profile for " + FourDInterface.currentUser
            );
        }
    }
    ...
    profileUpdated() {
        if (this.currentRecord) {
            ...
            this.currentRecord.refresh().then(rec => {
                ...
                if (this.viewerContentInfo)
                    this.viewerContentInfo.refreshGrid();
                if (this.profileRecommendationsInfo)
                    this.profileRecommendationsInfo.refreshGrid();
            });
        }
    }
}
