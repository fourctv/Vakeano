...
export class UserRating implements AfterViewInit {
    ...
    ngAfterViewInit() {
        ...
        if (!this.currentUser || this.currentUser <= 0)
            this.currentUser = FourDInterface.currentUserID;
        this.controlList.getRecords(
            <any>{
                custom: "MGSEFilterViewerContent",
                tableName: "Features",
                filter: "control",
                userID: this.currentUser,
                profileID: this.currentProfile
            },
            [Features.kFeatureId, Features.kIMDBTitle, Features.kPosterURL]
        );
    }
    ...
    rateThis(feature: Features, stars: number) {
        ...
        this.fourD
            .call4DRESTMethod("MGLErestUpdateViewerProfile", body)
            .subscribe(
                response => {
                    if (response.result === "OK") {
                        let list: Array<Features> = this.controlList.models;
                        this.controlList.models = list.filter(item => {
